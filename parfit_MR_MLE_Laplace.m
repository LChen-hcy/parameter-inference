function parfit_MR_MLE_Laplace
clc
close all
clear all
...
filename='result_MR_MLE_Laplace.xlsx';
...
for zushu=1:1:length(data(1,:))
    ...
    koff_e = nan;
    bs_e = nan;
    Sminexpression_LA_mean_fano = nan;
    else
    kbini = 5 + 49*rand;
    ini = [log(kbini)];
    [bminexpression_LA_mean_fano, Sminexpression_LA_mean_fano] = fminsearch(@Sfun_MR_MLE_Laplace,ini);  %call the negative log-likelihood function of telegraph model (FSP)
    kb_e = exp(bminexpression_LA_mean_fano)+fanodata+meandata-1;
    ...
    k = 1;
    AICc_TM = 2*Sminexpression_LA_mean_fano + 2*k + k*(k+1)/(N-k-1);
