...
filename='result_MR_MLE_FSP.xlsx';
...
for zushu=1:1:length(data(1,:))
    ...
    if fanodata <= 1
        ...
        SminFSP_mean_fano = nan;
    else
        kbini = 5 + 49*rand;
        ini = [log(kbini)];
        [bminFSP_mean_fano, SminFSP_mean_fano] = fminsearch(@Sfun_MR_MLE_FSP,ini);  %call the negative log-likelihood function of telegraph model (FSP)
        kb_e = exp(bminFSP_mean_fano)+fanodata+meandata-1;
    ...
    k = 1;
    AICc_TM = 2*SminFSP_mean_fano + 2*k + k*(k+1)/(N-k-1);
